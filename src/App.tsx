...
import ReactFlow, { Background, Connection, Controls, Edge, EdgeChange, MiniMap, Node, NodeChange, addEdge, applyEdgeChanges, applyNodeChanges, useReactFlow } from 'reactflow';
...
import {useMessage} from './component/Views/Message/Message';
...
function Main() {
  const [autoLayoutInProgress, setAutoLayoutInProgress] = React.useState<boolean>(false);
  ...
  const { screenToFlowPosition, getNode, zoomOut, zoomIn } = useReactFlow();
  ...
  const handleAutoLayout = () => {
    if (autoLayoutInProgress) return;
    setAutoLayoutInProgress(true);
    getLayoutedElements({ 'elk.algorithm': 'layered', 'elk.direction': 'RIGHT' });
    setTimeout(() => {
      setAutoLayoutInProgress(false);
    }, 500);
  ...
  React.useEffect(() => {
    ...
    if (autoLayoutInProgress) {
      message.open({type: 'loading', content: 'Loading in progress'
      });
    } else {
      message.destroy();
      message.success('Auto Routing Complete.', 1.5);
    }
  },[autoLayoutInProgress, messageApi]);
    ...
    <>
      ...
      <Container className='wrapper' ref={reactFlowWrapper}>
        ...
        >
          ...
          <Controls />
          <button
            style={{ zIndex: '200', position: 'relative', margin: '20px' }}
            onClick={() => { handleAutoLayout() }}>Re Layout</button>
          <MiniMap
            zoomable
            pannable
          />
